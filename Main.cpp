...
void movement(float edge_x[2], float edge_y[2], float speed, Vector2 &circle, float &circle_size)
{
    ...
    if (IsKeyDown(KEY_A) && (circle.x - circle_size) > edge_x[0])
    {
...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        // if (movementEnabled)
        // {
        movement(EDGE_X, EDGE_Y, speed, player, playerSize);
        // }
        ...
        if (!(camera_view.target.x - (playerSize / 2) - defaultOffset.x <= EDGE_X[0]) && movementEnabled) // < EDGE_X Min
        {
